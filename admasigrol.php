...
$result=mysqli_query($link,"select id,password from registro where id='$letra' group by id;");
...
$result2=mysqli_query($link,"insert into registro values('$letra','$rol','$password');");
$er=mysqli_error($link);
if($er!=0){
			die('error de conexion. '.$er);
               }
...
echo '<br>'.$rol;
echo '<br>'.$password;
header('location:admuserroles.php');
	}
	?>
	<br>
			<div class="relleno"></div>
